...
const fetchJobs = async (isNewSearch = false) => {
  try {
    ...
    // Reset jobs if it's a new search or filter change, otherwise append for infinite scroll
    setJobs((prevJobs) => (isNewSearch ? response.data.data : [...prevJobs, ...response.data.data]));
    setHasMore(response.data.data.length > 0); // Check if more jobs are available
  } catch (error) {
    console.error('Error fetching jobs:', error);
  }
};

// Apply filters and reset jobs list
const applyFilters = useCallback(() => {
  setJobs([]); // Clear the current job list
  setPage(1); // Reset to the first page for new search
  fetchJobs(true); // Fetch jobs based on new filters, passing true to indicate a new search
}, [filters, searchTerm]);
