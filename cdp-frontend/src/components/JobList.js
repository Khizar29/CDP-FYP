...
//   const [signInOpen, setSignInOpen] = useState(false);
//   const [searchTerm, setSearchTerm] = useState('');
//   const [filters, setFilters] = useState({ jobType: '', niche: '' });
//   const [selectedJob, setSelectedJob] = useState(null);
//   const [page, setPage] = useState(1);
//   const [hasMore, setHasMore] = useState(true);
//   const { user } = useContext(UserContext);
...
//                   {renderField(faBriefcase, job.job_type)}
//                   {renderField(faCalendarAlt, job.posted_on && new Date(job.posted_on).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }))}
