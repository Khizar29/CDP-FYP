...
const ProfilePage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Main Content */}
      <Box className="container mx-auto px-4 py-8 lg:py-12 lg:px-16 flex-grow">
        {/* Profile Card */}
        <Box className="bg-gray-100 shadow-lg rounded-lg p-8 mb-8 flex flex-wrap md:flex-nowrap items-center lg:gap-12">
          {/* Profile Image */}
          ...
          <Box className="md:flex-grow md:ml-8">
            <Typography variant="h3" component="h1" className="text-2xl lg:text-3xl font-bold">
              {alumni.firstName} {alumni.lastName}
            </Typography>
            <Typography variant="h6" component="h2" className="text-lg lg:text-xl font-semibold mt-2">
              {alumni.discipline} {/* Displaying discipline as the degree */}
            </Typography>
            <Typography variant="body1" className="mt-4 text-sm lg:text-base">
              {alumni.tagline || 'Aspiring professional seeking opportunities to make a positive impact.'}
            </Typography>

            {/* Contact Information */}
            <Box className="mt-6">
              <Typography variant="h5" component="h3" className="text-md lg:text-lg font-bold mb-2">
                Contact
              </Typography>
              <Typography variant="body1" className="text-sm lg:text-base">
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            ...
            <Box className="bg-white p-6 rounded-lg shadow-md mb-6">
              <Typography variant="h5" component="h3" className="text-lg lg:text-xl font-bold mb-4">
                Education
              </Typography>
              <Typography variant="body1" className="text-sm lg:text-base">
            ...
            <Box className="bg-white p-6 rounded-lg shadow-md">
              ...
              <Typography variant="body1" className="mt-2 text-sm lg:text-base">
          ...
          <Grid item xs={12} md={8}>
              ...
              {alumni.personalExperience ? (
                <ul className="list-disc pl-4">
                  {alumni.personalExperience.split(',').map((item, index) => (
                    <li key={index}>
                      <Typography variant="body1" className="text-sm lg:text-base">• {item.trim()}</Typography>
                    </li>
                  ))}
                </ul>
              ) : (
                <Typography variant="body1" className="text-sm lg:text-base">No work experience listed.</Typography>
              )}
            </Box>
            ...
            <Box className="bg-white p-6 rounded-lg shadow-md mb-6">
              ...
              {alumni.fyp ? (
                <Typography variant="body1" className="text-sm lg:text-base">{alumni.fyp}</Typography>
              ) : (
                <Typography variant="body1" className="text-sm lg:text-base">No FYP details available.</Typography>
              )}
            </Box>
              ...
              {alumni.certificate ? (
                <ul className="list-disc pl-4">
                  {alumni.certificate.split(',').map((certification, index) => (
                    <li key={index}>
                      <Typography variant="body1" className="text-sm lg:text-base">• {certification.trim()}</Typography>
                    </li>
                  ))}
                </ul>
              ) : (
                <Typography variant="body1" className="text-sm lg:text-base">No certifications listed.</Typography>
              )}
            </Box>
              ...
              {alumni.projects && alumni.projects.length > 0 ? (
                alumni.projects.map((project, index) => (
                  <Box key={index} className="mb-4">
                    <Typography variant="h6" component="h4" className="text-base lg:text-lg font-semibold">
                      {project.name}
                    </Typography>
                    <Typography variant="body1" className="text-sm lg:text-base">{project.description}</Typography>
                  </Box>
                ))
              ) : (
                <Typography variant="body1" className="text-sm lg:text-base">No projects listed.</Typography>
              )}
            </Box>
          </Grid>
        </Grid>
